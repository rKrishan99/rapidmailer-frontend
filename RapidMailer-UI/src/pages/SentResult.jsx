...
const SentResult = () => {
  const { loading, response, error } = useContext(EmailsSendContext);
    ...
    <div className="flex flex-col gap-8 p-6 md:p-10">
      ...
      {loading ? (
        ...
        <Card className="flex flex-col items-center gap-4 p-10 text-center">
          ...
          <h2 className="text-xl font-semibold text-white">Sending failed</h2>
        ...
        <Card className="flex flex-col items-center gap-4 p-10 text-center">
          ...
          <p className="max-w-md text-sm text-slate-400">{response || "Working on it..."}</p>
        </Card>
      )}
    </div>
  );
